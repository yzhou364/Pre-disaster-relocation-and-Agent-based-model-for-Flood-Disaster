...
ax.legend(['Not free rider',"Free rider"],loc='center left', bbox_to_anchor=(1, 0.5))
ax.set_xlabel("Residential discount rate (%)\n \n $750,000 relocation cost",fontsize =20)
ax.set_ylabel("House market value in millions",fontsize =20)
fmt = '${x:,.0f}'
...
#print(np.sum(df_1M.index*df_1M["alt"]))
#print(np.sum(df_1M["alt"]))


print(np.sum(df_1L.index*df_1L["alt"]))
print(np.sum(df_1L["alt"]))
