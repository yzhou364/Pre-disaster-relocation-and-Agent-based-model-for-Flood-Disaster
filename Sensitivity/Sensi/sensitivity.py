...
def sim(subsidy,goverDis,midResDis,lowResDis,midProperty,additionalMoveCost,housePriceCutoffPer,numofType):
    ...
    return [subsidy,governMent.objective,freeRiderNumber2,sum(totalNumberMovementList),totalNumberMovementList]
...
subsidyRange = [500000,610000,670000,720000]
#subsidyRange = np.linspace(0.05,0.5,10)
lowDisRateRange = [0.15,0.18,0.20,0.22]
...
for i in lowDisRateRange:
    ...
    runningIndex = 0
    f2 = open("lowDisExp_{ntype}_750K_15types.csv".format(ntype=i),"w", newline='')
    csvwriter = csv.writer(f2)
