...
for i in lowDisRateRange:
    for j in midDisRateRange:
        for k in housePriceCutRange:
            midResDis= j
            lowResDis = i
            housePriceCutoffPer = k
            resFile = open(r"C:\Users\yzhou364\Desktop\Dissertation\Experiment results\Res\Com\resident_objects_{midResDis}_{lowResDis}_{midProperty}_{additionalMoveCost}_{housePriceCutoffPer}_{numofType}.dat".format(midResDis=midResDis,lowResDis=i,midProperty=midProperty,additionalMoveCost = additionalMoveCost,housePriceCutoffPer=housePriceCutoffPer, numofType=numofType),"wb")
            resList = createRes(midResDis=j, lowResDis=i,midProperty=50000,additionalMoveCost = 300000,housePriceCutoffPer=k, numofType= 15)
