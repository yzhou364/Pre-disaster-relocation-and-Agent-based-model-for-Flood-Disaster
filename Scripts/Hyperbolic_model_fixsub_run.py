...
def sim(subsidy=10000,goverDis = 0.025,midResDis=0.12, lowResDis=0.18,midProperty=50000,
       additionalMoveCost = 300000,housePriceCutoffPer=50, numofType= 10,alpha = 0.001):
    """
    Data file path
    """
...
subsidyRange = np.linspace(500000,600000,11)
...
alphaTest = np.linspace(0.001, 0.025,25)
onerun = ["onerun"]
runningIndex = 0
runningParam = onerun
...
for j in subsidyRange :
    ...
    runningIndex = 0
    f2 = open(r"C:\Users\yzhou364\Desktop\Dissertation\Experiment results\NY_Res\5.6\losDisExp_{ntype}_750K_15types.csv".format(ntype=j),"w", newline='')
    csvwriter = csv.writer(f2)
    csvwriter.writerow(["Param","Objective","Number of free rider1","Number of free rider2","Total movement"])
