...
a = [mD,lD,hC]
import itertools
combina = list(itertools.product(*a))

for i in range(len(combina)):
    print("Agent:",i,"Created")
    midResDis = combina[i][0]
    lowResDis = combina[i][1]
    housePriceCutoffPer = combina[i][2]
    midProperty = 50000
    additionalMoveCost = 300000
    numofType =15
    resFile = open(r"C:\Users\yzhou364\Desktop\Dissertation\Experiment results\Res\Combination\resident_objects_{midResDis}_{lowResDis}_{midProperty}_{additionalMoveCost}_{housePriceCutoffPer}_{numofType}.dat".format(midResDis=midResDis,lowResDis=lowResDis,midProperty=midProperty,additionalMoveCost = additionalMoveCost,housePriceCutoffPer=housePriceCutoffPer, numofType=numofType),"wb")
    resList = createRes(midResDis=midResDis, lowResDis=lowResDis,midProperty=midProperty,additionalMoveCost = additionalMoveCost,housePriceCutoffPer=housePriceCutoffPer, numofType= numofType)
