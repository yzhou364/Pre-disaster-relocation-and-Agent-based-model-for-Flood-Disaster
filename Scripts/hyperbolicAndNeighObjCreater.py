...
distanceThres = np.linspace(1000,1500,6)
amc = [300000,750000]
    ...
    for j in amc:
        ...
        for res in resList:
            for neighbor in resList:
                if haversine((res.lat, res.long),(neighbor.lat, neighbor.long))*1000 <= i and haversine((res.lat, res.long),(neighbor.lat, neighbor.long))!=0:
